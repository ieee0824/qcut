...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  checkAndRecoverAutosave: async () => {
    try {
      const hasFile = await invoke<boolean>('has_autosave');
      if (!hasFile) return;
      ...
      if (recover) {
        ...
        const originalPath = project.metadata.originalPath ?? null;
