...
/// カラープリセットファイルを読み込む
#[tauri::command]
pub fn read_color_presets(app_handle: tauri::AppHandle) -> Result<String, String> {
    let app_data = app_handle
        .path()
        .app_data_dir()
        .map_err(|e| format!("app_data_dir の取得に失敗: {}", e))?;

    let presets_path = app_data.join("color-presets.json");
    ...
    fs::read_to_string(&presets_path)
        .map_err(|e| format!("カラープリセットファイルの読み込みに失敗: {}", e))
}

/// カラープリセットファイルを書き込む
#[tauri::command]
pub fn write_color_presets(app_handle: tauri::AppHandle, content: String) -> Result<(), String> {
    let app_data = app_handle
        .path()
        .app_data_dir()
        .map_err(|e| format!("app_data_dir の取得に失敗: {}", e))?;
